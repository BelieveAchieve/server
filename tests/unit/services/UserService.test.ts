...
describe('calculateHoursTutored', () => {
  ...
  for (const testCase of similarTestCases) {
    test(testCase, async () => {
      ...
      const result = await UserService.calculateHoursTutored(session);
...
describe('updateHoursTutored', () => {
  test('Should add hours tutored to user for sessions less than 3 hours', async () => {
    ...
    await UserService.updateHoursTutored(session._id);
    const { hoursTutored } = await getVolunteer(
      { _id: volunteer._id },
      { hoursTutored: 1 }
    );
